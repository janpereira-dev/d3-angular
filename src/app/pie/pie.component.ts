import { Component, OnInit } from '@angular/core';
import * as d3 from 'd3';

@Component({
    selector: 'app-pie',
    templateUrl: './pie.component.html',
    styleUrls: ['./pie.component.css'],
    standalone: false
})
export class PieComponent implements OnInit {

  private data = [
    {City: 'Caracas', Population: 1600000, Census: 2020},
    {City: 'Valencia', Population: 900000, Census: 2019},
    {City: 'Marcaibo', Population: 500000, Census: 2020},
    {City: 'Yaracuy', Population: 100000, Census: 2019},
    {City: 'Mérida', Population: 75000, Census: 2020},
    {City: 'Vargas', Population: 50000, Census: 2020}
  ];
  private svg;
  private margin = 50;
  private width = 750;
  private height = 600;
  // The radius of the pie chart is half the smallest side
  private radius = Math.min(this.width, this.height) / 2 - this.margin;
  private colors;

  constructor() { }

  ngOnInit(): void {
    this.createSvg();
    this.createColors();
    this.drawChart();
  }

  private createSvg(): void {
    const host = d3.select('figure#pie');
    host.select('svg').remove();

    this.svg = host
      .append('svg')
      .attr('width', this.width)
      .attr('height', this.height)
      .append('g')
      .attr(
        'transform',
        'translate(' + this.width / 2 + ',' + this.height / 2 + ')'
      );
  }

  private createColors(): void {
    this.colors = d3.scaleOrdinal()
    .domain(this.data.map(d => d.City))
    .range(['#b8d8f5', '#a7d9e8', '#9ed9d0', '#d7e8b7', '#f3d7a4', '#f5b7ab']);
  }

  private drawChart(): void {
    // Compute the position of each group on the pie:
    const pie = d3.pie<any>().value((d: any) => d.Population);
    const pieData = pie(this.data);

    // Build the pie chart
    const slices = this.svg
    .selectAll('pieces')
    .data(pieData)
    .enter();

    slices
    .append('path')
    .attr('d', d3.arc()
      .innerRadius(0)
      .outerRadius(this.radius)
    )
    .attr('fill', (d) => (this.colors(d.data.City)))
    .attr('stroke', '#f8fbff')
    .style('stroke-width', '1.5px')
    .style('opacity', 0.95);

    // Add labels
    const labelLocation = d3.arc()
    .innerRadius(100)
    .outerRadius(this.radius);

    slices
    .append('text')
    .text(d => d.data.City)
    .attr('transform', d => 'translate(' + labelLocation.centroid(d) + ')')
    .style('text-anchor', 'middle')
    .style('font-size', 14)
    .style('fill', '#374151')
    .style('font-weight', '600');
  }

}
